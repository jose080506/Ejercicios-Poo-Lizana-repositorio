...
void Pantalla::escribirLog( const QString & origen, const QString & descripcion )  {
    QFile archivo( rutaLog );
    if ( ! archivo.open( QIODevice::Append | QIODevice::Text ) )  return;

    QTextStream out( &archivo );
    QString fecha = QDateTime::currentDateTime().toString( "yyyy-MM-dd hh:mm:ss" );
    out << "[" << fecha << "] " << origen << ": " << descripcion << "\n";
    archivo.close();
}
